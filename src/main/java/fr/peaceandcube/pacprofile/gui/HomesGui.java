    ...
    protected void fillInventory() {
        ...
        for (int i = 0; i < Math.min(homesOnPage, 10); i++) {
            ...
            this.setItem(GuiItem.builder().slot(slot + 1).material(Material.PAPER)
                    ...
                    .lore(Component.empty(), this.getNotesLore(name), Component.empty())
                    .lore(LoreComponents.HOME_NOTES_CLICK_LEFT, LoreComponents.HOME_NOTES_CLICK_RIGHT)
                    .onLeftClick(() -> new AnvilGUI.Builder().plugin(PACProfile.getInstance())
                            .title(Messages.HOME_NOTES_TITLE)
                            .text(PACProfile.getInstance().playerData.getHomeNotes(this.player.getUniqueId(), name))
                            .itemLeft(new ItemStack(Material.PAPER))
                            .onClick((anvilSlot, stateSnapshot) -> {
                                if (anvilSlot != AnvilGUI.Slot.OUTPUT) {
                                    return List.of();
                                }
                                PACProfile.getInstance().playerData.setHomeNotes(stateSnapshot.getPlayer().getUniqueId(), name, stateSnapshot.getText());
                                new HomesGui(this.viewer, this.player, this.page, this.maxPages, this.orderSet.currentOrder()).open();
